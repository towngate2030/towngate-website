...
export function WhatsAppFab() {
  ...
  const prefill =
    locale === "ar"
      ? "مرحبًا TownGate، أود الاستفسار عن مشروعكم."
      : "Hi TownGate, I’d like to know more about your projects.";

  const href = useMemo(() => {
    if (first) {
      return `https://wa.me/${String(first).replace(/\D/g, "")}?text=${encodeURIComponent(
        prefill,
      )}`;
    }
