...
export function Hero({ locale, bgUrl, logoUrl, kicker, title, subtitle, primaryCta }: Props) {
  ...
  return (
    <section className="relative overflow-hidden bg-brand-navy text-tg-cream">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-24 md:pt-24">
        <motion.p
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          className="mb-4 hidden text-sm font-bold uppercase tracking-[0.2em] text-brand-orange md:block"
        >
          {kicker}
        </motion.p>
        <motion.h1
